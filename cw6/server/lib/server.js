...
Server.prototype.start = function(player_port, game_port) {
    ...
    this.game_server.on('initialised', function(id){self.game_id = id});
	...
	this.player_server.on('move', function(player, incoming_move){
		self.game_server.makeMove(player, incoming_move);
	});
}
...
Server.prototype.getNextColour = function() {
    //TODO
    var colour = this.colours.splice(0, 1);
    return colour[0];
}
